...
def fitness(netss):
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            bird.move()
            nets[x].fitness += 0.1
            output = nets[x].predict([bird.y, abs(bird.y - pipes[pipe_ind].height), abs(bird.y - pipes[pipe_ind].bottom)])
...
if __name__ == '__main__':
    ...
    g = group(20, fitness, 3, 5, 1, False, 0.01,0.2, 0.5, 3, save_to_file = True)
    g.run()
